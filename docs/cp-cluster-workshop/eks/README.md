...
### Storage Class
After running above script; we will get an EFS ID output like `fs-05ec0870ff96a6374`. We will need to use that value to deploy `dp-config-aws` helm chart.

```bash
## following variable is required to create the storage class
export CP_EFS_ID="fs-05ec0870ff96a6374" # replace with the EFS ID created in your installation
...
### Ingress Controller for TUNNEL Domain
```bash
...
helm upgrade --install --wait --timeout 1h --create-namespace --reuse-values \
  ...
  --labels layer=2 \
  --repo "${TIBCO_CP_HELM_CHART_REPO}" --version "1.0.17" -f - <<EOF
domain: ${DP_DOMAIN}
es:
  version: "8.9.1"
  ingress:
...
Please delete the Control Plane from TIBCO® Control Plane UI.
