# Istio Egress Gateway Helm Chart
...
To install the chart with the release name `istio-egressgateway`:
...
To uninstall/delete the `istio-egressgateway` deployment:

```console
helm delete istio-egressgateway
```
...
Istio historically offered two different charts - `manifests/charts/gateways/istio-egress` and `manifests/charts/gateways/istio-egress`.
...
* Designed with Helm best practices in mind (standardized values options, values schema, values are not all nested under `gateways.istio-egressgateway.*`, release name and namespace taken into account, etc).
...
* Single chart for all gateways (egress, Egress, East West).
...
```console
helm upgrade istio-egress manifests/charts/gateway -n istio-system --set name=istio-egressgateway --set labels.app=istio-egressgateway --set labels.istio=egressgateway
```
